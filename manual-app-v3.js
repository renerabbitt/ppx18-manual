...
function enableWorkingSearch(){const search=document.querySelector('#search'),count=document.querySelector('#count');function runSearch(q){const query=String(q||'').trim(),matches=[];[...nav.children].forEach((b,i)=>{const match=!query||chapterMatches(chapters[i],query);b.hidden=query?!match:false;if(match&&query)matches.push(i)});if(query){matches.sort((a,b)=>{const title=normalizeSearch(query);return Number(normalizeSearch(chapters[b].title).includes(title))-Number(normalizeSearch(chapters[a].title).includes(title))});count.textContent=matches.length+' RESULTS';rail.classList.add('open')}else count.textContent=chapters.length;return matches}search.oninput=e=>runSearch(e.target.value);search.onsearch=e=>runSearch(e.target.value);search.onkeydown=e=>{if(e.key==='Enter'){const matches=runSearch(e.target.value);if(matches.length)go(matches[0])}else if(e.key==='Escape'){search.value='';runSearch('');search.blur()}}}
